...
- [ ] **Step 3: Host app Info.plist is generated by XcodeGen**

Do NOT hand-write `App/Info.plist`. XcodeGen generates it at that path from the
target's `info.properties` (the `CFBundleURLTypes` / `LSUIElement` above) plus its
defaults. The generated `App/Info.plist` is gitignored (see Step 13) — `project.yml`
is the single source of truth. Hand-editing it would be overwritten on the next
`xcodegen generate`.
...
- [ ] **Step 10: Extension Info.plist is generated by XcodeGen**

Do NOT hand-write `Extension/Info.plist`. Its `CFBundlePackageType` and the
`NSExtension` dict are declared in the extension target's `info.properties` in
`project.yml` (Step 1), so XcodeGen writes them into the generated plist. This is
required for idempotency: if `NSExtension` were only in a hand-written file, the next
`xcodegen generate` would overwrite the file and drop it, breaking the extension. The
generated `Extension/Info.plist` is gitignored (Step 13).
...
> The `.xcodeproj` and both `Info.plist` files are generated from `project.yml`, so
...
> Because the Info.plists are gitignored, the tracked `App/` and `Extension/`
> directories contain only the `.swift` and `.entitlements` source files.
